classdef ADRev <  handle
    ...
    methods
        ...
        % Power - (This could be done if p is considered a parent, like 
        % e.g. the Strike Price of a Payoff. )
        ...
        function result = adr_payoff(X,K)
            ...
            for i=d:d % For-loop not necessary. Remove!
...
% --- Help functions -----------------------------------------------------%
function output = myDirac(x) % (Never used)
output = x;
for i=1:length(x)
    if x(i)==0
